...
/// throttling logic to limit the rate at which BackenStates release their admission
...
/// All backend states must eventually transition to RELEASED.
///
...
///     number of backends. The decay factor is controlled by
///     FLAGS_batched_release_decay_factor.
///
/// This class has an 'Init' method that must be called before any other method and a
/// 'Close' method that must be called before the object is destroyed. The 'Close' method
/// returns any remaining unreleased backends (e.g. backends in either the IN_USE or
/// PENDING state). Backends in the RELEASABLE state are assumed to be released by the
/// client, and any RELEASABLE backends must be marked as RELEASED by a call to
/// 'BackendsReleased' before the destructor is called. It is valid to call
/// 'MarkBackendFinished' or 'BackendsReleased' after the BackendResourceState is closed.
/// Once a BackendResourceState is closed, BackendStates can no longer transition to the
/// PENDING or RELEASABLE state.
///
...
class Coordinator::BackendResourceState {
 public:
  ...
  /// Initialize the BackendResourceState, must be called before any other methods.
  void Init();

  /// Mark a BackendState as finished and transition it to the PENDING state. Returns
  /// a list of RELEASABLE states that should be released by the caller and then passed
  /// to BackendsReleased. Returns an empty list if no pending backends should be
  /// released.
  ...
  /// BackendStates have been released.
  void BackendsReleased(const std::vector<BackendState*>& released_backend_states);

  /// Closes that state machine and returns a vector of IN_USE or PENDING BackendStates.
 ...
 private:
  ...
  /// Counts the number of backends in the PENDING, RELEASABLE, or RELEASED state.
  int num_pending_releasable_or_released_ = 0;

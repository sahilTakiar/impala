...
class BufferedTupleStream {
 public:
  ...
  /// Initializes the tuple stream object on behalf of caller 'caller_id'. Must be called
  /// once before any of the other APIs.
  /// If 'pinned' is true, the tuple stream starts off pinned, otherwise it is unpinned.
  /// 'caller_id' is only used for error reporting.
  Status Init(const std::string &caller_id, bool pinned) WARN_UNUSED_RESULT;

  /// The same as 'Status Init(const std::string &caller_id, bool pinned)' except it
  /// takes an int as the 'caller_id'. The 'caller_id' is converted to a string.
  Status Init(int caller_id, bool pinned) WARN_UNUSED_RESULT;
  ...
  /// Returns the number of bytes currently unpinned in memory by the stream.
  int64_t BytesUnpinned() const {
 ...
 private:
  ...
  /// BufferedTupleStream. For ExecNodes this is the TPlanNodeId.

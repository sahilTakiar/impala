...
void Coordinator::BackendResourceState::MarkBackendFinished(
  ...
  if (!closed_
    ...
    ++num_pending_;
    ++num_pending_releasable_or_released_;
    ...
    bool is_coordinator_the_last_unreleased_backend = !released_coordinator_
        && num_pending_releasable_or_released_ == num_backends_ - 1;
    ...
    // PENDING backend states to releasable and update any state necessary for the
    // heuristics.
...
void Coordinator::BackendResourceState::BackendsReleased(
  ...
  for (auto backend_state : released_backend_states) {
    DCHECK(backend_resource_states_.at(backend_state) != ResourceState::RELEASED);

...
class CoordinatorBackendStateTest : public testing::Test {
 protected:
  ...
  /// to the supplied vector. 'coordinator_backend' points to backend running the
...
///     * Releasing all backends transitions all states to RELEASED.
...
TEST_F(CoordinatorBackendStateTest, CoordinatorOnly) {
  ...
  // 'Timeout Release' heuristic should not be triggered.
  ...
  // Release the last non-coordinator backend, this should trigger all PENDING backends

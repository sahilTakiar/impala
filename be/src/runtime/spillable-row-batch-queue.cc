...
Status SpillableRowBatchQueue::Open() {
  ...
  // buffer pages.
  batch_queue_ = new BufferedTupleStream(state_, row_desc_,
...
Status SpillableRowBatchQueue::AddBatch(RowBatch* batch) {
  if (UNLIKELY(IsFull())) return Status("SpillableRowBatchQueue is full.");
  if (UNLIKELY(closed_)) return Status("SpillableRowBatchQueue is already closed.");
  Status status;
...
Status SpillableRowBatchQueue::GetBatch(RowBatch* batch) {
  if (UNLIKELY(IsEmpty())) return Status("SpillableRowBatchQueue is empty.");
  if (UNLIKELY(closed_)) return Status("SpillableRowBatchQueue is already closed.");
  bool eos = false;
...
bool SpillableRowBatchQueue::IsFull() const {
  ...
  if (UNLIKELY(closed_)) return false;
...
bool SpillableRowBatchQueue::IsEmpty() const {
  ...
  if (UNLIKELY(closed_)) return true;

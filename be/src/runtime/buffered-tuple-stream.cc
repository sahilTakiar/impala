...
Status BufferedTupleStream::Init(const string &caller_id, bool pinned) {
  if (!pinned) UnpinStream(UNPIN_ALL_EXCEPT_CURRENT);
  caller_id_ = caller_id;
  return Status::OK();
}

Status BufferedTupleStream::Init(int caller_id, bool pinned) {
  return Init(std::to_string(caller_id), pinned);
}
...
Status BufferedTupleStream::CalcPageLenForRow(int64_t row_size, int64_t* page_len) {
  if (UNLIKELY(row_size > max_page_len_)) {
    return Status(TErrorCode::MAX_ROW_SIZE,
        PrettyPrinter::Print(row_size, TUnit::BYTES), caller_id_,

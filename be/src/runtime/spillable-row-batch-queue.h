...
class SpillableRowBatchQueue {
 public:
  ...
  /// successfully added, returns an error Status if the queue is full, has already been
  /// closed, or if there was an error when adding the batch to the BufferedTupleStream.
  /// If adding the batch to the BufferedTupleStream cannot be achieved because there is
  /// no more available reserved memory, this method will unpin the stream and then add
  /// the RowBatch. If the batch still cannot be added, this method returns an error
  /// Status.
  Status AddBatch(RowBatch* batch);

  /// Returns and removes the RowBatch at the head of the queue. Returns Status::OK() if
  /// the batch was successfully read from the queue. Returns an error status if the
  /// queue is empty or has already been closed.
  Status GetBatch(RowBatch* batch);

  /// Returns true if the queue limit has been reached, false otherwise. If the queue is
  /// already closed, returns false.
  bool IsFull() const;

  /// Returns true if the queue is empty, false otherwise. If the queue is already closed,
  /// returns true.
  bool IsEmpty() const;

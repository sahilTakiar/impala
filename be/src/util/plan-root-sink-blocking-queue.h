...
#ifndef IMPALA_UTIL_PLAN_ROOT_SINK_BLOCKING_QUEUE_H
#define IMPALA_UTIL_PLAN_ROOT_SINK_BLOCKING_QUEUE_H
...
template <typename T>
class PlanRootSinkBlockingQueue : public CacheLineAligned {
 public:
  PlanRootSinkBlockingQueue(size_t max_elements)
    : shutdown_(false),
    ...
    DCHECK_NE(offsetof(PlanRootSinkBlockingQueue, put_bytes_enqueued_) / 64,
        offsetof(PlanRootSinkBlockingQueue, get_lock_) / 64);
  }
 ...
 private:
  ...
  /// True if the PlanRootSinkBlockingQueue is being shut down. Guarded by 'put_lock_'.
  bool shutdown_;

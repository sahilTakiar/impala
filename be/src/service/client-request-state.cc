...
void ClientRequestState::SetOriginalId(unique_ptr<TUniqueId> original_id) {
  original_id_ = move(original_id);
...
Status ClientRequestState::InitExecRequest(const TQueryCtx& query_ctx) {
  return UpdateQueryStatus(
      ExecEnv::GetInstance()->frontend()->GetExecRequest(query_ctx, exec_request_));
}

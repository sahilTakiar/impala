...
ClientRequestState::ClientRequestState(
    ...
    unique_ptr<const TUniqueId> retried_id)
...
void ClientRequestState::Wait() {
  ...
  DCHECK(exec_state == ExecState::FINISHED || exec_state == ExecState::ERROR
      || exec_state == ExecState::RETRYING || exec_state == ExecState::RETRIED);
...
Status ClientRequestState::RestartFetch() {
  ...
  if (result_cache_.get() == NULL) {
    ...
    return Status(ErrorMsg(TErrorCode::GENERAL, ss.str())).SetIsRecoverable();
  }
...
void ClientRequestState::UpdateNonErrorExecState(ExecState new_state) {
  ...
  switch (new_state) {
    ...
    case ExecState::RETRIED:
      DCHECK(old_state == ExecState::RETRYING);
      UpdateExecState(new_state);
      break;
    default:
...
Status ClientRequestState::UpdateQueryStatus(const Status& status) {
  ...
  if (!status.ok() && query_status_.ok()) {
    ExecState exec_state = exec_state_.Load();
    bool is_retryable = exec_request().query_options.retry_failed_queries
        && status.IsRetryable() && exec_state != ExecState::RETRYING;
    if (is_retryable) {
      ...
      // Only queries in specific ExecStates can be retried.
      ...
      DCHECK(exec_state == ExecState::INITIALIZED || exec_state == ExecState::PENDING
          || exec_state == ExecState::RUNNING)
          << Substitute("Illegal state: $0", ExecStateToString(exec_state_.Load()));

      // Update the ExecState and schedule a retry of the query.
...
Status ClientRequestState::Cancel(bool check_inflight, const Status* cause) {
...

  {
    ...
    if (!already_done && cause != NULL) {
      ...
      ExecState exec_state = exec_state_.Load();
      DCHECK(exec_state == ExecState::ERROR || exec_state == ExecState::RETRYING
          || exec_state == ExecState::RETRIED);
    }
...
void ClientRequestState::UpdateExecState(ExecState exec_state) {
  ...
  if (exec_state == ExecState::RETRYING) {
    retry_exec_state_ = old_state;
  }
  summary_profile_->AddInfoString("Query State", PrintThriftEnum(BeeswaxQueryState()));
...
beeswax::QueryState::type ClientRequestState::BeeswaxQueryState() const {
  return ExecStateToBeeswaxQueryState(exec_state());
}

TOperationState::type ClientRequestState::TOperationState() const {
  return ExecStateToOperationState(exec_state());
}
...
Status ClientRequestState::LoadExecRequest(const TQueryCtx& query_ctx) {
...
void ClientRequestState::WaitUntilRetried() {
  unique_lock<mutex> l(lock_);
  ExecState exec_state = exec_state_.Load(); // Load the exec state once
...
string ClientRequestState::ExecStateToString(ExecState state) const {
  static const unordered_map<ClientRequestState::ExecState, const char*>
      exec_state_strings{{ClientRequestState::ExecState::INITIALIZED, "INITIALIZED"},
          ...
          {ClientRequestState::ExecState::UNKNOWN, "UNKNOWN"}};

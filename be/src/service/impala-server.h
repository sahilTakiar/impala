...
class ImpalaServer : public ImpalaServiceIf,
 ...
 private:
  ...
  /// of query. Creates ClientRequestState (returned in query_driver), registers it and
  ...
  /// Registers the query driver with query_driver_map_ using the globally unique
  /// query_id. The caller must have checked out the session state.
  ...
  struct QueryStateRecord {
    ...
    /// query_state = union(beeswax::QueryState, ClientRequestState::RetryState).
  ...
  /// Return the QueryHandle for this query id. The QueryHandle contains the active
  /// ClientRequestState. Returns an error Status if the query id cannot be found.
  Status GetQueryHandle(
  ...
  Status SetupResultsCacheing(ClientRequestState* request_state,

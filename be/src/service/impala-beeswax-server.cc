...
void ImpalaServer::query(beeswax::QueryHandle& beeswax_handle, const Query& query) {
  ...
  TUniqueIdToBeeswaxHandle(request_state->query_id(), &beeswax_handle);
}
...
void ImpalaServer::fetch(Results& query_results,
  ...
  QueryHandle query_handle;
  RAISE_IF_ERROR(GetQueryHandle(query_id, &query_handle), SQLSTATE_GENERAL_ERROR);

  // Validate that query can be accessed by user.
  RAISE_IF_ERROR(CheckClientRequestSession(session.get(),
                     query_handle.request_state->effective_user(), query_id),
      SQLSTATE_GENERAL_ERROR);
  Status status =
...
void ImpalaServer::get_log(string& log, const LogContextId& context) {
  ...
  // Validate that query can be accessed by user.
  RAISE_IF_ERROR(
      CheckClientRequestSession(session.get(), request_state->effective_user(), query_id),
      SQLSTATE_GENERAL_ERROR);

...
Status ImpalaServer::GetRuntimeProfileOutputFromHandle(const string& user,
...
Status ImpalaServer::GetQueryRecord(const TUniqueId& query_id, QueryLogIndex::const_iterator* query_record) {
...
// need a way to do the mapping + a way to get what exactly what you want
// maybe split into two methods?
Status ImpalaServer::GetRuntimeProfileOutput(const TUniqueId& query_id,
    const string& user, TRuntimeProfileFormat::type format, RuntimeProfileOutput* original_profile,
  ...
  // Search for the query id in the active query map
  {
    ...
    // QueryHandle of the original query if the query is retried.
    QueryHandle original_query_handle;
    Status status = GetAllQueryHandles(query_id, &active_query_handle, &original_query_handle,
        /*return_unregistered=*/true);

    if (status.ok()) {
      // Set the profile for the retried query.
      DCHECK(original_query_handle->WasRetried());
      // Set the profile for the original query.
      RETURN_IF_ERROR(
          GetRuntimeProfileOutputFromHandle(user, active_query_handle, format, retried_profile));

      RETURN_IF_ERROR(GetRuntimeProfileOutputFromHandle(
          user, original_query_handle, format, original_profile));
      return Status::OK();
    }
  }
  ...
  // The query was not found the active query map, search the query log.
  {
    ...
    // Set the profile for the retried query.
    DCHECK(query_record->second->was_retried);
    ...
    RETURN_IF_ERROR(GetQueryRecord(*query_record->second->retried_query_id, &retried_query_record));
    ...
    DCHECK(CheckProfileAccess(user, retried_query_record->second->effective_user,
...
Status ImpalaServer::GetRuntimeProfileOutput(const TUniqueId& query_id,
  ...
  // Search for the query id in the active query map
  {
    ...
    if (status.ok()) {
      RETURN_IF_ERROR(
          GetRuntimeProfileOutputFromHandle(user, query_handle, format, profile));
      return Status::OK();
    }
  }

...
Status ImpalaServer::GetRuntimeProfileOutput(const TUniqueId& query_id,
  ...
  // Search for the query id in the active query map
  {
    shared_ptr<QueryDriver> query_driver =
        GetQueryDriver(query_id, /*return_unregistered=*/ true);
...
Status ImpalaServer::Execute(TQueryCtx* query_ctx, shared_ptr<SessionState> session_state,
  ...
  if (!status.ok() && registered_request_state) {
    UnregisterQueryDiscardResult(query_handle->request_state->query_id(), false, &status);
  }
  return status;
}

Status ImpalaServer::ExecuteInternal(const TQueryCtx& query_ctx,
    shared_ptr<SessionState> session_state, bool* registered_request_state,
    QueryHandle* query_handle) {
  DCHECK(session_state != nullptr);
  *registered_request_state = false;

  query_handle->query_driver.reset(new QueryDriver(this));
  query_handle->query_driver->CreateClientRequestState(
      query_ctx, session_state, query_handle);
  ClientRequestState* request_state = query_handle->request_state;
...

  {
    ...
    RETURN_IF_ERROR(
        RegisterQuery(query_ctx.query_id, session_state, query_handle->query_driver));
...
Status ImpalaServer::RegisterQuery(const TUniqueId& query_id,
    shared_ptr<SessionState> session_state, shared_ptr<QueryDriver> query_driver) {
  ...
  RETURN_IF_ERROR(query_driver_map_.AddQueryDriver(query_id, query_driver));
...
Status ImpalaServer::SetQueryInflight(
    shared_ptr<SessionState> session_state, ClientRequestState* request_state) {
...
void ImpalaServer::UpdateExecSummary(ClientRequestState* request_state) const {
...
Status ImpalaServer::UnregisterQuery(const TUniqueId& query_id, bool check_inflight,
  ...
  QueryHandle query_handle;
  RETURN_IF_ERROR(GetQueryHandle(query_id, &query_handle));

  // Try to start unregistration on this thread. Once StartUnregister() returns, the
  // query is considered unregistered from the client's point of view. If
  // StartUnregister() returns OK, this thread is responsible for doing the
  // unregistration work. StartUnregister() succeeds for the first thread to call it to
  // avoid multiple threads unregistering.
  RETURN_IF_ERROR(query_handle.query_driver->StartUnregister());

  // Do the work of unregistration that needs to be done synchronously.
  RETURN_IF_ERROR(query_handle.request_state->Finalize(check_inflight, cause));
  ...
  unreg_thread_pool_->Offer(query_handle);
  return Status::OK();
}
...
void ImpalaServer::FinishUnregisterQuery(const QueryHandle& query_handle) {
  ...
  CloseClientRequestState(query_handle.request_state);
...
void ImpalaServer::CloseClientRequestState(ClientRequestState* request_state) {
...
shared_ptr<QueryDriver> ImpalaServer::GetQueryDriver(
  ...
  if (!return_unregistered && entry->second->started_unregister()) {
...
Status ImpalaServer::GetQueryHandle(
  ...
  query_handle->query_driver = GetQueryDriver(query_id);
  if (UNLIKELY(query_handle->query_driver == nullptr)) {
    Status err = Status::Expected(TErrorCode::INVALID_QUERY_HANDLE, PrintId(query_id));
    VLOG(1) << err.GetDetail();
    return err;
  }
  query_handle->request_state = query_handle->query_driver->GetActiveClientRequestState();
  DCHECK(query_handle->request_state != nullptr);
  return Status::OK();
}
...
Status ImpalaServer::CancelInternal(const TUniqueId& query_id) {
  ...
  RETURN_IF_ERROR(GetQueryHandle(query_id, &query_handle));
  RETURN_IF_ERROR(
      query_handle.request_state->Cancel(/*check_inflight=*/ true, /*cause=*/ nullptr));
  return Status::OK();
}
...
Status ImpalaServer::WaitForResults(const TUniqueId& query_id,
  ...
  QueryHandle handle;
  RETURN_IF_ERROR(GetQueryHandle(query_id, &handle));
  *query_handle = handle;
  ClientRequestState* request_state = query_handle->request_state;
  BlockOnWait(request_state, timed_out, block_on_wait_time_us);
  ...
  retry_state = request_state->retry_state();
  if (retry_state == ClientRequestState::RetryState::RETRYING
    ...
    RETURN_IF_ERROR(GetQueryHandle(query_id, &handle));
    *query_handle = handle;
    request_state = query_handle->request_state;
    ...
    DCHECK(retry_state != ClientRequestState::RetryState::RETRYING
        && retry_state != ClientRequestState::RetryState::RETRIED)
        << "Unexpected state: " << request_state->RetryStateToString(retry_state);
  }
  return Status::OK();
}

void ImpalaServer::BlockOnWait(ClientRequestState* request_state,
...
void ImpalaServer::CancelFromThreadPool(const CancellationWork& cancellation_work) {
  ...
  if (cancellation_work.unregister()) {
    ...
    if (cancellation_work.cause() == CancellationWorkCause::BACKEND_FAILED) {
      // Create an error status with the TErrorCode RETRYABLE_ERROR.
      Status retryable_status =
          Status::Expected(TErrorCode::RETRYABLE_ERROR, error.msg().msg());
      for (const string& s : error.msg().details()) retryable_status.AddDetail(s);

      Status retry_status =
          query_driver->TryQueryRetry(request_state, &retryable_status, &was_retried);
      if (!retry_status.ok()) {
        UnregisterQueryDiscardResult(query_id, true, &retry_status);
      }
    }
...
void ImpalaServer::QueryStateRecord::Init(const ClientRequestState& request_state) {
  ...
  beeswax_query_state = request_state.BeeswaxQueryState();
  ...
  if (retry_state == ClientRequestState::RetryState::NOT_RETRIED) {
    ...
    query_state = request_state.RetryStateToString(retry_state);
  }
...
void ImpalaServer::UpdateFilter(UpdateFilterResultPB* result,
  ...
  shared_ptr<QueryDriver> query_driver =
      GetQueryDriver(ProtoToQueryId(params.query_id()));
  if (query_driver == nullptr) {
  ...
  ClientRequestState* client_request_state =
      query_driver->GetClientRequestState(ProtoToQueryId(params.query_id()));
  DCHECK(client_request_state != nullptr);
  ClientRequestState::RetryState retry_state = client_request_state->retry_state();
  if (retry_state != ClientRequestState::RetryState::RETRYING
      && retry_state != ClientRequestState::RetryState::RETRIED) {

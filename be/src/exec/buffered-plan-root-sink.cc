...
Status BufferedPlanRootSink::GetNext(
  ...
  // acquire the lock. It is safe to call notify is_full_ regardless of whether a
  // RowBatch is read. Either (1) a RowBatch is read and the queue is no longer full, so
  // notify the consumer thread or (2) a Rowbatch was not read, which means either

...
Status BlockingPlanRootSink::FlushFinal(RuntimeState* state) {
  ...
  // All rows have been sent by the producer, so wake up the producer so it can set eos to true.
  consumer_cv_.NotifyAll();
  return Status::OK();
}
...
Status BlockingPlanRootSink::GetNext(
  ...
  // Set the shared QueryResultSet pointer 'results_' to the given 'results' object and wake
  // up the sender thread so it can add rows to 'results_'.
  results_ = results;

...
class BufferedPlanRootSink : public PlanRootSink {
 public:
  BufferedPlanRootSink(TDataSinkId sink_id, const RowDescriptor* row_desc,
      RuntimeState* state, RowBatchQueue* batch_queue);
 ...
 private:
  ...
  /// A generic RowBatchQueue that buffers RowBatches from the sender for consumption by
  /// the consumer.
  RowBatchQueue* batch_queue_ = nullptr;
};
}

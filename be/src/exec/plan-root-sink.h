...
class PlanRootSink : public DataSink {
 public:
  ...
  /// 'timeout' is the amount of time this method should wait for enough rows to become
  /// available before returning (in microseconds) (e.g. how long the consumer thread waits
  ...
  virtual Status GetNext(RuntimeState* state, QueryResultSet* result_set, int num_rows,
      bool* eos, uint64_t timeout_us) = 0;

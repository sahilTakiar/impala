...
class TestQueryStates(ImpalaTestSuite):
  ...
  def test_query_states(self):
    ...
    assert "Impala Query State: PENDING" in profile
    ...
    assert "Impala Query State: FINISHED" in profile, profile

  def test_error_query_state(self):
    """Tests that the query profile shows the proper error state."""
    query = "select id from functional_parquet.bad_column_metadata"
    handle = self.execute_query_async(query, {"abort_on_error": "1"})

    def assert_finished():
      profile = self.client.get_runtime_profile(handle)
      return "Query State: FINISHED" in profile and \
             "Impala Query State: FINISHED" in profile
    ...
    profile = self.client.get_runtime_profile(handle)
    assert "Query State: ERROR" in profile
    assert "Impala Query State: ERROR" in profile

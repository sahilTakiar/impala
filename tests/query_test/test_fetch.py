...
class TestFetch(ImpalaTestSuite):
  ...
  def test_rows_sent_counters(self, vector):
    ...
    RowMaterializationTimer, and RowsMaterialized are set to valid values in
    the ImpalaServer section of the runtime profile."""
    num_rows = 10
    ...
    assert "RowsMaterialized: {0} ({0})".format(num_rows) in result.runtime_profile
